...
from models import Post, Order, Testimonial, CalendarEvent, User, WeddingPackage
from forms import PostForm, TestimonialForm, CalendarEventForm, UserEditForm, WeddingPackageForm
...
def admin_panel():
    ...
    calendar_events = CalendarEvent.query.filter(
        (CalendarEvent.order_id.isnot(None)) | (CalendarEvent.is_available == False)
    ).all()
    ...
    formatted_events = [
        {
            ...
            'color': '#28a745' if event.order_id else '#dc3545' # Green for approved orders, red for unavailable
        }
    ...
    ).order_by(Order.event_date.asc()).all()
...
def edit_profile_admin():
    ...
    return render_template('edit_profile.html', form=form, user=current_user, title='Edit Admin Profile')
...
def delete_client(user_id):
    ...
    try:
        # Delete associated orders and their related data (calendar events, payment proofs)
        for order in user.orders:
            if order.dp_payment_proof:
                file_path = os.path.join(current_app.root_path, 'static/payment_proofs', order.dp_payment_proof)
        ...
        # Delete associated testimonials (if any are directly linked to user and not via order)
        # Note: With current model, testimonials are linked to order, so this might be redundant if cascade works.
        # However, explicit deletion ensures all related data is handled.
...
def client_list():
    ...
    return render_template('admin/client_list.html', clients=clients, search_query=search_query)
...
def order_list():
    ...
    return render_template('admin/orders_list.html', orders=orders, search_query=search_query, status_filter=status_filter)
...
def approve_order(order_id):
    ...
    if order.status not in ['waiting_approval', 'cancelled']:
        flash('Order cannot be approved. It is not in waiting_approval or cancelled status.', 'danger')
        return redirect(url_for('admin.order_list'))
    order.status = 'accepted'
...
def reject_order(order_id):
    ...
    order = Order.query.get_or_404(order_id)
    if order.status not in ['waiting_approval', 'cancelled']:
        flash('Order cannot be rejected. It is not in waiting_approval or cancelled status.', 'danger')
        return redirect(url_for('admin.order_list'))
...
DTSTART:{dtstart}
DTEND:{dtend}
SUMMARY:Photo Session for {order.client.full_name} ({order.service_type.capitalize()})
LOCATION:{order.location}
DESCRIPTION:Details: {order.details or 'N/A'}. Client WhatsApp: {order.client.whatsapp_number}
END:VEVENT
END:VCALENDAR"""
...
def new_portfolio_post():
    ...
    if request.method == 'POST':
        ...
        if not images or not images[0].filename: # Check if any file is actually uploaded
            return jsonify({'success': False, 'message': 'Please upload at least one image.'}), 400

        if not (1 <= len(images) <= 10):
            return jsonify({'success': False, 'message': 'Please upload between 1 and 10 images.'}), 400
        ...
        return jsonify({'success': True, 'message': 'Your post has been created!', 'redirect_url': url_for('admin.manage_portfolio')})

    return render_template('admin_post_form.html', form=form, title='New Portfolio Post')
    ...
    if request.method == 'POST':
        ...
        for img_id in images_to_delete_ids:
            ...
            if image_to_delete:
                ...
                image_path = os.path.join(current_app.root_path, 'static/images', image_to_delete.filename)
        ...
        if uploaded_new_images:
            if not (1 <= (current_image_count + len(uploaded_new_images)) <= 10):
                db.session.rollback()
                return jsonify({'success': False, 'message': 'Total images (existing + new) must be between 1 and 10.'}), 400
        ...
        if not uploaded_new_images and not post.images:
            ...
            return jsonify({'success': False, 'message': 'A post must have at least one image. Please upload new images or uncheck delete for existing ones.'}), 400

        post.title = request.form.get('title')
        post.content = request.form.get('content')
        db.session.commit()

        return jsonify({'success': True, 'message': 'Your post has been updated!', 'redirect_url': url_for('admin.manage_portfolio')})

    elif request.method == 'GET':
        form.title.data = post.title
        form.content.data = post.content
    return render_template('admin_post_form.html', form=form, title='Edit Portfolio Post', post=post)
...
def calendar():
    ...
    formatted_events = [
        {
            'title': event.title,
            'start': event.start_time.strftime('%Y-%m-%dT%H:%M:%S'), # ISO 8601 format for FullCalendar
            'end': event.end_time.strftime('%Y-%m-%dT%H:%M:%S'), # ISO 8601 format for FullCalendar
            'description': event.description,
            'color': '#28a745' if event.order_id else '#dc3545', # Green for approved orders, red for unavailable
...
def new_calendar_event():
    ...
    if form.validate_on_submit():
        event = CalendarEvent(title=form.title.data,
    ...
    return render_template('admin/create_edit_calendar_event.html', form=form, title='New Calendar Event')
...
def edit_calendar_event(event_id):
    ...
    return render_template('admin/create_edit_calendar_event.html', form=form, title='Edit Calendar Event')
...
def new_testimonial():
    ...
    if form.validate_on_submit():
        testimonial = Testimonial(client_name=form.client_name.data,
    ...
    return render_template('admin/create_edit_testimonial.html', form=form, title='New Testimonial')
...
def edit_testimonial(testimonial_id):
    ...
    return render_template('admin/create_edit_testimonial.html', form=form, title='Edit Testimonial')
...
def new_wedding_package():
    ...
    if form.validate_on_submit():
        package = WeddingPackage(name=form.name.data, description=form.description.data, price=form.price.data)
        db.session.add(package)
        db.session.commit()
        flash('Wedding package created successfully!', 'success')
        return redirect(url_for('admin.wedding_package_list'))
    return render_template('admin/create_edit_wedding_package.html', form=form, title='New Wedding Package')
...
def edit_wedding_package(package_id):
    ...
    return render_template('admin/create_edit_wedding_package.html', form=form, title='Edit Wedding Package')
