from flask import render_template, Blueprint, redirect, url_for, flash, jsonify, current_app
...
def order():
    ...
    wedding_packages = {pkg.id: pkg.price for pkg in wedding_packages_list} # Convert to dict for JS

    if form.validate_on_submit():
        # Backend validation for date availability
        requested_date = form.event_date.data
        try:
            start_time_obj = datetime.strptime(form.event_start_time.data, '%H:%M').time()
        ...
        # Check if there's an accepted/completed order or an unavailable calendar event that overlaps
        existing_event = CalendarEvent.query.filter(
            (CalendarEvent.start_time < full_end_datetime) & (CalendarEvent.end_time > full_start_datetime),
            or_(CalendarEvent.is_available == False, CalendarEvent.order_id.isnot(None))
        ).first()

        if existing_event:
            flash('This date and time slot is already booked or unavailable. Please choose another time.', 'danger')
            return render_template('order.html', form=form, wedding_packages=wedding_packages)
        ...
        order = Order(client_id=current_user.id,
                      ...
                      wedding_package_id=selected_wedding_package_id, # Assign selected package ID
                      status='waiting_dp') # Set initial status to waiting_dp
        ...
        flash('Your order has been placed successfully! Please proceed to DP payment.', 'success')
        return redirect(url_for('client.dp_payment', order_id=order.id)) # Redirect to DP payment page
