...
try:
    ...
    ImageDraw = None
    ImageFont = None
    print("Pillow is not installed. Please install it using 'pip install Pillow' to generate dummy images.")
...
def generate_dummy_portfolio(num_posts, images_per_post):
    ...
    if Image is None:
        print("Pillow is not installed. Cannot generate dummy images. Please install it using 'pip install Pillow'.")
        return
    ...
    with app.app_context():
        print(f"Generating {num_posts} dummy posts with {images_per_post} images each...")
        for i in range(num_posts):
            title = f"Dummy Post {i+1}"
            content = f"This is dummy content for post {i+1}. It showcases some of our amazing work."
